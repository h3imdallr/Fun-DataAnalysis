...
def detect_ts(df, max_anoms=0.10, direction='pos',
    ...
    """ Checking Parameters """
    check_parameters(df, max_anoms, direction,alpha, only_last, threshold,
              e_value, longterm, piecewise_median_period_weeks, plot,
              y_log, xlabel, ylabel,title, verbose)
    ...
    for i in range(len(all_data)):
        ...
        """ (1) decompose data using detect_anoms() """
        s_h_esd_timestamps = detect_anoms(all_data[i], k=max_anoms, alpha=alpha, num_obs_per_period=period,
                                          use_decomp=True, one_tail=anomaly_direction.one_tail,
                                          upper_tail=anomaly_direction.upper_tail, verbose=verbose,
                                          resample_period = resample_period #ymjung
                                          )
        ...
        """ (2) Use detected anomaly timestamps to extract the actual anomalies (timestamp and value) from the data """
        # (2-1) anomaly detected by S-H-ESD(by detect_anoms)
        ...
        # (2-2) anomaly detected by threshold if applicable
        if threshold:
    ...
    """ (Optional)Add additional column containing expected value. """
    if e_value:
        d = {
...
def check_parameters(df, max_anoms=0.10, direction='pos', alpha=0.05, only_last=None, threshold=None,
              e_value=False, longterm=False, piecewise_median_period_weeks=2, plot=False,
              y_log=False, xlabel = '', ylabel = 'count', title=None, verbose=False):
